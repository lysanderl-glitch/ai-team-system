...
## E2E 测试能力说明（2026-04-23 赋能）

CDE 现具备完整的 MEOS JDG E2E 测试执行能力，无需研发介入即可独立完成：

| 场景 | 操作 | 预期输出 |
|------|------|----------|
| 客户验收前全量回归 | 执行 `run-jdg-report.bat` | 中文健康度报告（pass/fail/模块状态） |
| 版本上线后冒烟验证 | 同上 | 同上，约 15 分钟出结果 |
| 特定模块快速验证 | Claude Code 对话触发单文件 | 即时结果 |
| 发现 Bug 时 | 截取失败输出 → 标准 bug 报告模板 | 可直接提交研发 |

**操作入口**：
- 快速操作：`meos-e2e/run-jdg-report.bat`（双击运行）
- 完整手册：`meos-e2e/docs/cde-runbook.md`
- 技术归档：`meos-e2e/docs/jdg-e2e-archive.md`
