...
#### Golden Path: 对已完成任务执行 QA 门禁

- **场景名称**：M/L 级任务交付前执行完整 6 维质量评审
- **输入**：`/qa-gate Skill test_scenarios 补充任务`
- **前置条件**：
  - 当前会话中有已完成的工作（有文件变更）
  - 变更文件中包含 YAML 或代码文件（用于技术质量检查）
- **预期结果**：
  - [ ] 分析审查对象，确定本次审查范围
  - [ ] 逐项评分 6 个维度：目标达成度 / 执行链完整性 / 技术质量 / 知识沉淀 / 风险控制 / 功能端到端完整性
  - [ ] 技术质量维度：对 YAML 文件执行语法检查（`python3 -c "import yaml; yaml.safe_load(...)"` 或等效验证）
  - [ ] 功能端到端完整性维度：检查交付物是否定义了 test_scenarios（`grep -r "test_scenarios"` ）
  - [ ] 输出完整评分表格格式：维度 / 得分 / 说明
  - [ ] 总分计算正确（6 项之和，满分 6.0）
  - [ ] 结论判定正确：>= 4.2 通过 / 3.6-4.1 条件通过 / < 3.6 不通过
  - [ ] 工具调用链：`Read(变更文件) -> Bash(YAML检查) -> Grep(test_scenarios检查) -> 输出评分`

#### Edge Case 1: 交付物缺少 test_scenarios 时的评分降级

- **场景名称**：审查的 Skill 类交付物未包含 test_scenarios 定义
- **输入**：`/qa-gate 某个未定义测试场景的 Skill`
- **前置条件**：
  - 交付物为 Skill 类文件
  - 文件中不包含 `test_scenarios` / `golden path` / `edge case` 关键词
- **预期结果**：
  - [ ] 维度 6（功能端到端完整性）评分 <= 0.3
  - [ ] 评分说明明确标注"无 test_scenarios 定义"
  - [ ] 如果其他维度得分正常，总分可能因此低于合格线
  - [ ] 结论中明确指出需补充 test_scenarios 才能通过

#### Edge Case 2: 无明确审查对象时审查当前会话

- **场景名称**：不带参数调用时自动审查当前会话工作
- **输入**：`/qa-gate`（无参数）
- **前置条件**：
  - 当前会话有已完成工作
- **预期结果**：
