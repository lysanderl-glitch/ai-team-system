...
```
① 智囊团共识        与智囊团对齐目标、需求，制定策略和方案
        ↓
② 方案确认          向总裁提交方案，获得明确批准
        ↓
③ 执行团队共识      向执行团队说明目标、需求、验收标准
        ↓
④ 执行              执行团队完成任务
        ↓
⑤ 智囊团 + QA 审查  审查任务完成质量，评审通过后才进入下一步
        ↓
⑥ 决策上报（如需）  若有需要总裁决策的事项，先经智囊团判断是否必要上报，再上报总裁
```

### 执行规则

- **任何任务**开始前，必须先完成 ① → ②，不得直接跳到执行
- **执行完成后**，必须经过 ⑤ 审查，审查通过才能推进
- **需要决策时**，必须先走 ⑥ 的智囊团前置过滤，而非直接打扰总裁
- **违反执行链**的行为视为执行失误，需要重新从当前缺失环节补齐
