...
print(f'Total codes mapped: {len(new_map)}')

# Load original WBS records
with open('wbs_records.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

# Check all codes are covered
all_codes = set(r['WBS编码'] for r in data)
mapped_codes = set(new_map.keys())
missing = all_codes - mapped_codes
extra = mapped_codes - all_codes
print(f'Missing from new_map: {missing}')
print(f'Extra in new_map (not in wbs_records): {extra}')

# Build parent relations
def get_parent_code(code, level, all_codes_set):
    if level == 1:
        return None
    if level == 2:
        ...
        prefix2 = code[:2]
        if prefix2 in all_codes_set:
            return prefix2
        return None
    elif level == 4:
        if code[-1].isalpha() and not code[-2].isalpha():
            parent = code[:-1]
            if parent in all_codes_set:
                return parent
        return None
    return None

records_by_code = {r['WBS编码']: r for r in data}
...
for r in data:
    code = r['WBS编码']
    level = r['层级']
    page_id = new_map.get(code)
    if not page_id:
        print(f'WARNING: no new page_id for {code}')
        continue

    parent_code = get_parent_code(code, level, mapped_codes)
    if parent_code:
        parent_page_id = new_map.get(parent_code)
        if parent_page_id:
    ...
    pred_raw = r.get('前置依赖', '')
    if pred_raw:
        preds = [p.strip() for p in pred_raw.split(',') if p.strip()]
        pred_pages = []
        for p in preds:
            pp = new_map.get(p)
            if pp:
                pred_pages.append(pp)
            else:
                print(f'WARNING: predecessor {p} for {code} not in new_map')
        if pred_pages:
            predecessor_relations.append({
                'page_id': page_id,
                'code': code,
                'pred_codes': preds,
                'pred_page_ids': pred_pages
            })

print(f'Parent relations: {len(parent_relations)}')
print(f'Predecessor relations: {len(predecessor_relations)}')
...
with open('new_wbs_relations.json', 'w', encoding='utf-8') as f:
    json.dump(result, f, ensure_ascii=False, indent=2)
print('Saved new_wbs_relations.json')
